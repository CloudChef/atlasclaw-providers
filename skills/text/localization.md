...
Use this reference only when the requested TXT, Markdown, or HTML output involves translation or localization. For provider-specific UI strings, catalogs, or product terminology, prefer provider documentation or a supplied glossary.
...
Use context when it is available. If exact localization matters and the context is missing, ask one concise follow-up; otherwise state reasonable assumptions in the final answer.
