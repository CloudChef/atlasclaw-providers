...
from smartcmp_provider.domain.cost import normalize_money

def _currency() -> str:
    """Return no guessed currency when verified evidence is unavailable."""
...
def build_recommendation_analysis_payload(
    ...
    *,
    ...
    """Build the complete adapter-neutral cost recommendation assessment."""

    normalized_violation = enrich_violation_with_resource_context(
        violation,
        resource_records,
    )
    normalized_violation["monthlyCost"] = normalize_money(
        violation.get("monthlyCost")
    )
    normalized_violation["monthlySaving"] = normalize_money(
        violation.get("monthlySaving")
    )
    ...
    saving_contribution = None
    if saving_summary and facts.get("monthlySaving"):
        optimizable = (
            saving_summary.get("optimizableAmount")
            or saving_summary.get("currentMonthOptimizable")
            or 0
        )
        if optimizable:
            saving_contribution = round(
                facts["monthlySaving"] / optimizable * 100,
                2,
            )
    operation_type = str(
        facts.get("savingOperationType") or ""
    ).upper()
    risk = (
        "high"
        if operation_type == "TEAR_DOWN_IN_RESOURCE"
        else "low"
        if operation_type in {"CHANGE_PAY_TYPE", "SWITCH_TO_SUBSCRIPTION"}
        else "medium"
    )
    ...
    return {
        ...
        "assessment": {
            "optimizationTheme": theme,
            "violationType": violation_type,
            "cloudBestPractice": BEST_PRACTICE_GUIDANCE.get(
                theme,
                BEST_PRACTICE_GUIDANCE["manual_review"],
            ),
            "cloudBestPracticeZh": BEST_PRACTICE_GUIDANCE_ZH.get(
                theme,
                BEST_PRACTICE_GUIDANCE_ZH["manual_review"],
            ),
            "executionReadiness": readiness,
            "taskDefinitionPresent": bool(
                facts.get("taskDefinitionName")
            ),
            "savingSummaryAvailable": bool(saving_summary),
            "operationSummaryAvailable": bool(operation_summary),
            "savingContributionPct": saving_contribution,
            "isTopSavingResource": any(
                isinstance(item, dict)
                and str(item.get("resourceId") or "")
                == str(facts.get("resourceId") or "")
                for item in resource_top or []
            ),
            "riskLevel": risk,
            "policyComplianceRate": (
                latest_execution.get("complianceRate")
                or latest_execution.get("compliance")
            ),
            "violationRecurrence": facts.get("times", 0),
            "relatedPolicyCount": related_policy_count,
            "resourceContextAvailable": bool(
                facts.get("resourceContextAvailable")
            ),
            "resourceFetchStatus": facts.get(
                "resourceFetchStatus",
                "",
            ),
        },
        ...
        "suggestedNextStep": (
            str(recommendations[0].get("action") or "manual_review")
            if recommendations
            else "manual_review"
        ),
    }
...
def enrich_violation_with_resource_context(
    violation: dict,
    resource_records: list | None,
) -> dict:
    """Attach bounded normalized resource facts to a cost violation."""

    enriched = dict(violation)
    projected_records = project_resource_records(resource_records)
    requested_ids: list[str] = []
    original_id = str(enriched.get("resourceId") or "").strip()
    if original_id:
        requested_ids.append(original_id)
    for record in projected_records:
        resource_id = str(record.get("resourceId") or "").strip()
        if resource_id and resource_id not in requested_ids:
            requested_ids.append(resource_id)
    context = {
        "requestedResourceIds": requested_ids,
        "resolvedCount": sum(
            1
            for record in projected_records
    ...
    primary = next(
        ...
        projected_records[0] if projected_records else {},
    )
    enriched["resourceContext"] = context
    enriched["resourceContextAvailable"] = context["resolvedCount"] > 0
    enriched["resourceFetchStatus"] = primary.get("fetchStatus", "")
    if primary.get("fetchStatus") == "ok":
        enriched["resourceId"] = (
            primary.get("resourceId") or enriched.get("resourceId") or ""
        )
        enriched["resourceName"] = (
            enriched.get("resourceName") or primary.get("name") or ""
        )
        enriched["resourceType"] = (
            primary.get("resourceType")
            or (primary.get("normalized") or {}).get("type")
            or enriched.get("resourceType")
            or ""
        )
        enriched["componentType"] = primary.get("componentType", "")
        enriched["resourceStatus"] = primary.get("status", "")
        enriched["osType"] = primary.get("osType", "")
        enriched["osDescription"] = primary.get("osDescription", "")
    return enriched


def project_resource_records(resource_records: list | None) -> list[dict]:
    """Project resource evidence without credentials or unbounded raw payloads."""
        ...
        projected.append(
            {
                ...
                "name": summary.get("name") or resource.get("name") or "",
                "resourceType": (
                ...
                "componentType": (
                    summary.get("componentType")
                    or resource.get("componentType")
                    or ""
                ),
                "status": (
                    summary.get("status") or resource.get("status") or ""
                ),
                "osType": (
                    summary.get("osType") or resource.get("osType") or ""
                ),
                "osDescription": (
                    summary.get("osDescription")
                    or resource.get("osDescription")
                    or ""
                ),
                "fetchStatus": record.get("fetchStatus", ""),
                "missingEvidence": list(
                    record.get("missingEvidence") or []
                ),
