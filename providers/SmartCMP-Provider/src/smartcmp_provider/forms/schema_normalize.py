...
from smartcmp_provider.forms.catalog_fields import resolve_catalog_field_alias
from smartcmp_provider.forms.schema_field_normalize import (
    infer_type,
...
from smartcmp_provider.forms.schema_js_fields import iter_field_javascript
from smartcmp_provider.forms.schema_js_validate import (
    validate_javascript_expression,
    validate_value_expression_contract,
)
...
def _normalize_root_fieldsets(
    ...
    if not isinstance(fieldsets, list):
        normalized["fieldsets"] = (
            [_default_root_fieldset(list(properties.keys()))]
            if properties
            else []
        )
    ...
    for index, fieldset in enumerate(fieldsets):
        ...
        if isinstance(fieldset.get("fields"), list):
            normalized_fields = _normalize_fieldset_fields(fieldset["fields"], properties)
            if normalized_fields != fieldset["fields"]:
                fieldset["fields"] = normalized_fields
                warnings.append(
                    "Normalized root fieldset fields to schema property keys "
                    f"for fieldset at index {index}."
                )
            continue
        legacy_properties = fieldset.get("properties")
        if isinstance(legacy_properties, list):
            fields = _normalize_fieldset_fields(legacy_properties, properties)
            if fields:
...
def _normalize_fieldset_fields(fields: list[Any], properties: dict[str, Any]) -> list[str]:
...
def _normalize_top_level_field(
    ...
    value_config = config.get("value") if isinstance(config, dict) else None
    expression = value_config.get("expression") if isinstance(value_config, dict) else None
    if isinstance(expression, str):
        warnings.extend(validate_value_expression_contract(expression, field_key=field_key))
    for js_field_key, expression in iter_field_javascript(field, field_key):
        warnings.extend(
            validate_javascript_expression(
                expression,
                field_key=js_field_key,
