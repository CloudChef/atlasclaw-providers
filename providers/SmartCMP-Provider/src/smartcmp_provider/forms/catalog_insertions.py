...
"""Insert service-catalog standard fields into form schemas."""
...
from smartcmp_provider.forms.catalog_fields import (
    build_catalog_field_schema,
    resolve_catalog_field_alias,
)
...
def apply_catalog_fields(schema: dict[str, Any], catalog_fields_json: str) -> list[str]:
    ...
    for request in requests_json:
        ...
        try:
            ...
            hidden_is_explicit = isinstance(hidden_value, bool)
            hidden = hidden_value if hidden_is_explicit else False
            ...
            continue
        explicit_replace_keys = {"hidden"} if hidden_is_explicit else set()
...
def _merge_existing_catalog_field(
    ...
    for key, value in existing.items():
        ...
        if key not in field:
            if key in explicit_replace_keys:
                continue
