...
"""Resolve declarative value-expression source specifications."""
...
def parse_projection_fields(
    raw_fields: Any,
    warnings: list[str],
    allowed_model_paths: set[str] | None = None,
) -> list[dict[str, Any]]:
    """Normalize declarative projection fields into verified labels and paths."""
    ...
    for raw_field in raw_fields:
        ...
        _verify_value_path(path, allowed_model_paths)
...
def resolve_compose_spec(
    ...
    if isinstance(value, list):
        return [
            resolve_compose_spec(item, warnings, output_label, allowed_model_paths)
            for item in value
        ]
    ...
    if "$field" in value and set(value).issubset(helper_keys | {"$field"}):
        ...
        return _source_reference_from_field_alias(
            ...
            label=label,
            warnings=warnings,
            allowed_model_paths=allowed_model_paths,
        )
    ...
    return {
        key: resolve_compose_spec(
            item,
            warnings,
            key if isinstance(key, str) else output_label,
            allowed_model_paths,
        )
...
def _source_reference_from_field_alias(
    ...
    *,
    ...
    if not isinstance(value, str) or not value.strip():
        ...
        _verify_value_path(str(value or ""), allowed_model_paths)
    ...
    _verify_value_path(value.strip(), allowed_model_paths)
...
def _verify_value_path(value: Any, allowed_model_paths: set[str] | None) -> None:
    values = value if isinstance(value, list) else [value]
    for path in values:
        if not isinstance(path, str) or not path.strip():
            continue
        stripped = path.strip()
        if "." in stripped or stripped in (allowed_model_paths or set()):
            continue
        raise ValueError(
            f"Unverified value expression path {stripped!r}; use field/$field "
            "for catalog aliases or add the source field to schema.properties."
        )
