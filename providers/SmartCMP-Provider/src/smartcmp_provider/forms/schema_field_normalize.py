...
from smartcmp_provider.forms.schema_js_validate import validate_javascript_expression


def infer_type(field: dict[str, Any]) -> str:
    """Infer the JSON schema type from one SmartCMP widget definition."""
...
def normalize_widget(field: dict[str, Any], field_key: str, warnings: list[str]) -> None:
    """Normalize one field widget in place and record each deterministic repair."""
...
def is_table_array_field(field: dict[str, Any]) -> bool:
    """Return whether a field uses SmartCMP's table-array representation."""
...
def normalize_visibility(field: dict[str, Any], field_key: str, warnings: list[str]) -> None:
    """Ensure one field declares request and approval visibility defaults."""
...
def normalize_array_field(field: dict[str, Any], field_key: str, warnings: list[str]) -> None:
    """Normalize a SmartCMP table field and its nested item schema in place."""
