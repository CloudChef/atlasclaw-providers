...
def _parse_fields(value: str) -> list[tuple[str, str]]:
    ...
    for raw in re.split(r",+", normalized):
        ...
        if key.startswith("@request:"):
            raise SystemExit(
                "Catalog fields must use exact keys resolved from service catalog detail; "
                "fixed request-context aliases are not supported."
            )
        pair = (key, label)
...
def _build_expression(
    ...
    return (
        ...
        "var text=function(v){if(v===null||v===undefined)return'';if(typeof v==='object')v=v.label||v.name||v.displayName||v.text||v.value||v.username||v.loginId||v.originName||v.id||'';return String(v).replace(/\\s+/g,' ').replace(/^\\s+|\\s+$/g,'');};"
        "var domText=function(v){var raw=(v===null||v===undefined)?'':String(v),parts=raw.split(/[\\r\\n]+/),out=[],p;for(var i=0;i<parts.length;i++){p=text(parts[i]);if(p)out.push(p);}return out.length?out[out.length-1]:text(raw);};"
        "var state=W[ID]||{};state.values=state.values||{};var seen=[];"
        ...
        "var valueOf=function(k,label){seen=[];var v=byKey(k);if(v){state.values[k]=v;return v;}var rs=roots();for(var i=0;i<rs.length;i++){v=catalogValue(rs[i],k)||deep(rs[i],k,0);if(v){state.values[k]=v;return v;}}v=byLabel(label,k);if(v){state.values[k]=v;return v;}return state.values[k]||'';};"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.hide_submit_field_pos and _is_bool_literal(fieldset_title):
        ...
        fieldset_title = fieldset_title or "Form Fields"
