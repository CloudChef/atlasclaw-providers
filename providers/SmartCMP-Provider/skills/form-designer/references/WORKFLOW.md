...
When the user asks to change a form from a URL, start with `smartcmp_read_form_schema`. If the requested change alters the field set, field schema shape, layout, visibility, or custom JavaScript behavior, use the source schema only to understand the existing form and the user's reference point, then call `smartcmp_design_form_schema` with `mode=regenerate` and a fresh complete replacement schema from the user's latest requirements. Do not splice partial changes into the old JSON, do not preserve old fields or JavaScript by default, and do not ask the LLM to hand-copy a long existing schema.

If the requested URL-based change is a deterministic projection/composition update, such as filling one target field from service-catalog values, call `smartcmp_design_form_schema` with `mode=modify`, the exact `form_url`, and `value_expressions_json`. The tool loads the source schema, replaces the target expression, and emits warnings when the source form includes visual-designer components or stale `content.model` keys.

`smartcmp_design_form_schema` with `mode=modify` requires either complete `schema_json` or a source `form_url`. Do not call modify mode with only insertion/update JSON; an insertion without a source schema is not a complete form replacement.
