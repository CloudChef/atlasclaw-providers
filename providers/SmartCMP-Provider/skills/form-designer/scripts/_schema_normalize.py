...
try:
    from _catalog_context_sync import build_catalog_context_sync_expression
except ModuleNotFoundError as exc:
    if exc.name != "_catalog_context_sync":
        raise
    _catalog_context_sync = _load_module_from_path(
        Path(__file__).with_name("_catalog_context_sync.py"),
        "_smartcmp_form_designer_catalog_context_sync",
    )
    build_catalog_context_sync_expression = (
        _catalog_context_sync.build_catalog_context_sync_expression
    )
...
def _normalize_auto_sync_submission_shape(
    ...
    if "schemaFormValid" not in properties:
        ...
        properties["schemaFormValid"] = _normalize_top_level_field(
            "schemaFormValid",
            properties["schemaFormValid"],
            len(properties) - 1,
            warnings,
...
def _normalize_mock_auto_sync_field(
    ...
    if isinstance(expression, str):
        ...
        expression = value.get("expression")
        if isinstance(expression, str):
            _replace_handwritten_catalog_context_expression(
                field_key,
                value,
                expression,
                warnings,
...
def _warn_mock_auto_sync_expression_antipatterns(
    ...
    if (
        ...
        warnings.append(
            f"Mock auto-sync expression for field {field_key!r} appears to copy a historical IP-form expression verbatim; adapt KEY/output labels and FIELD_SPECS for the target service-catalog fields."
        )
    ...
    if re.search(r"valueOf\s*=\s*function\s*\(\s*name\s*,\s*rootsList\s*\)", expression):
        warnings.append(
            f"Mock auto-sync expression for field {field_key!r} has a valueOf helper that ignores the requested output name; use per-output key lists from the maintained catalog context template."
        )
...
def _replace_handwritten_catalog_context_expression(
    field_key: str,
    value: dict[str, Any],
    expression: str,
    warnings: list[str],
) -> None:
    """Replace hand-written fixed catalog FIELD_SPECS with maintained template.

    Args:
        field_key: Root schema property key for the field.
        value: Mutable `config.value` object containing the expression.
        expression: JavaScript function string to inspect.
        warnings: Mutable warning list describing deterministic repairs.
    """
    outputs = _fixed_catalog_context_outputs_from_expression(expression)
    if not outputs:
        return

    value["expression"] = build_catalog_context_sync_expression(field_key, outputs)
    warnings.append(
        "Replaced hand-written catalog context expression for field "
        f"{field_key!r} with maintained CATALOG_CONTEXT_SYNC_TEMPLATE_V1; "
        "use catalog_context_sync_json instead of hand-writing JavaScript."
    )


def _fixed_catalog_context_outputs_from_expression(expression: str) -> list[str]:
    """Return fixed catalog output labels from a non-template FIELD_SPECS expression.

    Args:
        expression: JavaScript function string from `config.value.expression`.

    Returns:
        Ordered output labels when all FIELD_SPECS outputs are supported fixed
        service-catalog labels; otherwise an empty list.
    """
    if (
        "FIELD_SPECS" not in expression
        or "CATALOG_CONTEXT_SYNC_TEMPLATE_V1" in expression
    ):
        return []
    ...
    for field_spec_source in _field_spec_sources(expression):
        ...
        if output not in _STRICT_DISPLAY_OUTPUT_KEYS:
            return []
