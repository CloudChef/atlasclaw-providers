...
- PDF text layers and validated DOCX/PPTX/XLSX text are extracted locally. Legacy DOC/PPT/XLS files use LibreOffice when its `libreoffice` or `soffice` command is installed and otherwise return a specific availability error. Direct image attachments use the configured visual-capable Agent model to produce searchable Markdown; BMP input is normalized to PNG in a resource-limited local worker while the original BMP remains in the Vault. Embedded Office images are not visually analyzed, and PDFs without extractable text are rejected.

Local document extraction uses the provider's `python-docx`, `python-pptx`, `openpyxl`, and `pypdf` dependencies. LibreOffice is optional and used only to convert legacy DOC/PPT/XLS files; Poppler is not required. Linux/macOS advisory file locking coordinates REST writes with read-only chat tools; cancelled lock waiters close their descriptors, and marker-validated transaction journals recover interrupted swaps. Direct image attachments require a model that accepts image input through Core's generic visual bridge.
