...
Attachment conversion is synchronous and atomic. Validated DOCX/PPTX/XLSX and PDF text layers are extracted locally; legacy DOC/PPT/XLS files are rejected until an OS-level sandbox is available. Images and PDF/Office pages without a usable text layer are sent to the configured visual-capable Agent model for OCR and diagram interpretation. A visual conversion failure rejects the complete snapshot and preserves the previous version.

Office conversion requires LibreOffice, scanned-page rendering requires Poppler `pdftoppm`, and PDF text extraction requires the provider's `pypdf` dependency. The complete Knowledge aggregate shares one conversion deadline; page count, rendered bytes, generated assets, and cumulative Markdown size are bounded by provider instance configuration. Linux/macOS advisory file locks prevent REST and chat readers from observing a directory swap. Interrupted swaps are recovered only from marker-validated entries in `.atlasclaw-transactions/`.
